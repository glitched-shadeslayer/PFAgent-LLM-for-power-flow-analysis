...
from models.llm_only_schema import PowerFlowResultSchema, TotalsSchema
...
def test_llm_only_dispatcher_uses_current_net_when_matpower_file_is_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("llm.tools._serialize_net_to_matpower_text", lambda *_, **__: "SERIALIZED_FROM_NET")
    monkeypatch.setattr(
        "llm.tools._get_matpower_meta_from_net",
        lambda *_: MatpowerMeta(total_load_mw_ref=90.0, zero_rateA_line_ids={1}, slack_bus_id_ref=1),
    )
    ...
    def fake_solve(**kwargs):
        ...
        return PowerFlowResultSchema(
            ...
            bus_voltages=[
                {"bus_id": 1, "vm_pu": 1.0, "va_deg": 0.0},
                {"bus_id": 2, "vm_pu": 0.99, "va_deg": -5.0},
            ],
            line_flows=[
                {"line_id": 1, "from_bus": 1, "to_bus": 2, "p_from_mw": 10.0, "q_from_mvar": 1.5, "loading_percent": 0.0}
            ],
            voltage_violations=[],
            thermal_violations=[],
            summary_text="ok",
    ...
    monkeypatch.setattr("llm.tools._solve_from_matpower_text", fake_solve)
    ...
    assert "error" not in pf_out
    assert pf_out.get("case_name") == "case14"
    assert captured["matpower_text"] == "SERIALIZED_FROM_NET"
    assert captured["m_file_path"] == "<serialized:case14>"
    assert isinstance(captured["matpower_meta"], MatpowerMeta)
